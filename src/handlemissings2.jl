"""
    @handlemissings2(fun, collectskipped=false, definedefault=false)
...
- `@traitfun fun(x1::::IsEltypeSuperOfMissing, ::PassMissing, ...)` 
  ...
  Otherwise, it convhandlemissingsrts the type of each element to the corresponding nonmissing type
...
# Notes  
For extending or overwriting the defintitions created by `@handlemissings2` the
...
@handlemissings2(frealvec, true, true) # defines several new methods
...
```
"""
macro handlemissings2(FUN, collectskipped=false, definedefault=false)
...
Similar to [`@handlemissings2`](@ref) 
...
Similar to `@handlemissings2` but with additionally specifying the position of the
...
macro handlemissings_pos(FUN, pos_strategy, collectskipped=false, definedefault=false)
  pos_strategy < 3 :(error("for positions 1 or two use handlemissings1 or handlemissings2"))
